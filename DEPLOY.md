...
### ИИ-Дизайн: через Google Gemini

Раздел «ИИ-Дизайн» на сайте генерирует 3 фотореалистичных ракурса дома
(фасад спереди, сбоку, в перспективе) по текстовому описанию клиента через
Google Gemini (модель `gemini-3.1-flash-image-preview`, настраивается через
`GEMINI_IMAGE_MODEL`). План этажа считается отдельно формулой (не ИИ) —
см. `buildFloorPlan()` в `index.html`, он точный и не требует API-ключа.

На Railway → сервис backend → Variables добавьте `GEMINI_API_KEY`:
1. Зайдите на https://aistudio.google.com/apikey, создайте ключ (бесплатно,
   без привязки карты).
2. Вставьте его в переменную `GEMINI_API_KEY` на Railway.

Если ключа нет — генерация фото вернёт понятную ошибку вместо падения
сервера. У Gemini есть бесплатный дневной лимит запросов — для старта
хватает; при росте нагрузки нужно включить платный биллинг в Google Cloud
(это дешевле OpenAI: ориентировочно $0.04 за картинку).

### Настройка ИИ-консультанта поддержки (Claude)

Раздел «Поддержка → Чат» на сайте отвечает клиентам через Claude (Anthropic
API), опираясь на реальные FAQ магазина из базы данных — вместо старого
поиска ответа по совпадению слов.

На Railway → сервис backend → **Variables** добавьте:
```
ANTHROPIC_API_KEY=sk-ant-...
```
(необязательно) `ANTHROPIC_MODEL` — если нужна другая модель Claude, по
умолчанию используется быстрая и дешёвая `claude-haiku-4-5-20251001`.

Если ключ не задан или Anthropic временно недоступен — чат автоматически
откатывается на старый поиск по FAQ, сайт не падает.
