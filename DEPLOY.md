...
### ИИ-Дизайн: чертёж (план комнат) через Claude

Раздел «ИИ-Дизайн» на сайте строит чертёж дома — список комнат с площадью,
нарисованный как SVG-схема — по ПОЛНОМУ текстовому описанию клиента (не
только площадь и число спален, а вообще всё: нужен гараж, кабинет, большая
кухня-гостиная и т.п.). Генерация — через Claude (Anthropic API), тот же
ключ `ANTHROPIC_API_KEY`, что и у ИИ-консультанта поддержки (см. ниже) —
отдельный ключ для этого раздела не нужен.

Важно: Claude — языковая модель, она не рисует фотореалистичные картинки.
Раздел возвращает именно чертёж (структурированные данные о комнатах),
а не иллюстрацию «как дом выглядит снаружи».

### Настройка Claude (ИИ-консультант поддержки + ИИ-Дизайн)

Один и тот же ключ Anthropic используется в двух местах:
- «Поддержка → Чат» — отвечает клиентам, опираясь на реальные FAQ магазина
  из базы данных, вместо старого поиска ответа по совпадению слов;
- «ИИ-Дизайн» — строит чертёж дома по описанию клиента (см. выше).
...
Если ключ не задан или Anthropic временно недоступен:
- ИИ-консультант автоматически откатывается на старый поиск по FAQ;
- ИИ-Дизайн вернёт понятную ошибку вместо падения сервера.
